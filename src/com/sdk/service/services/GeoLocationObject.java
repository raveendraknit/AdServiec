...
Query  q=session.createSQLQuery("Select * FROM Tblbanners tb INNER JOIN Tblbannerlocations tbl ON tbl.bannerId = tb.bannerId INNER JOIN Tbllocations tl ON tl.locationId = tbl.locationID"
...
+" AND (tbT.bannerId is null OR :currentDate between tbT.startTime and tbT.endTime) and tb.archived = B'0' ORDER BY tb.priority DESC, (tb.impressions + 1) * (100 - tb.priority) ASC, tb.impressions ASC").addEntity(Tblbanners.class).addEntity(Tbllocations.class).addEntity(Tblbannerlocations.class);
        ...
        q.setParameter("longitude", location.getLongitudeInDegrees());
      	...
      	q.setInteger("companyid", Integer.parseInt(pubReference));
//        q.setBoolean("enable", true);
...
//           q.setBoolean("archived", false);
	...
	public static void main(String[] args) throws BusinessException {
		...
		GeoLocation myLocation = GeoLocation.fromDegrees(-38.687599,41.93733);
		double distance = 100;
		Session session = HibernateUtil.getSession();
        SortedSet<TblAdvRefTemp> resultList = findPlacesWithinDistance(
				earthRadius, myLocation, distance, session,"24");
