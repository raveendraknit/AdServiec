...
/** pretty print a chunk of code.
  ...
  */
function prettyPrintOne(sourceCodeHtml,json) {

  if(json)
    sourceCodeHtml = formatCode(sourceCodeHtml);
    ...
    for (; k < elements.length && new Date().getTime() < endTime; k++) {
      ...
      if (cs.className && cs.className.indexOf('prettyprint') >= 0) {
        ...
        if (!nested) {
          ...
          var newContent = prettyPrintOne(content,json);
...
function formatCode(code){
  ...
  (function reCur(){
    var chr = codeArr.shift();
    if(chr==="{" || chr==="[") {
      stack.push(1)
      chr+="\n"+getTabs(stack.length);
    } else if(chr==="}" || chr==="]") {
      stack.pop(1)
      chr="\n"+getTabs(stack.length)+chr;
    } else if(chr===",") {
      chr+="\n"+getTabs(stack.length);
    } 
    str+=chr;
