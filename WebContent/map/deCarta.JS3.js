...
deCarta.Utilities = {
    ...
    getTileBoundingBox: function(e,n,z){
        e = parseFloat(e);
        n = parseFloat(n);
        z = parseFloat(z);
        var size = deCarta.Utilities.tileSizeForZoom(z);
        var topLeft = deCarta.Utilities.getTilePosition(e,n,z);
        var btmRight = new deCarta.Core.Position(0,0);
        btmRight.setXY(topLeft.getX(z)+size, topLeft.getY(z)-size, z);
        
        return new deCarta.Core.BoundingBox(topLeft, btmRight);
    },
    /**
    ...
    */
     ...
     * @param {Integer} dist -  meters - required
     * @return {String} returns string in format: P0DT0H0M0S
    */
    podPed:function(dist){
        var total = dist * .72;
    ...
    /**
     * multiply time estimate by a given factor   
     * @param {String} pod - required
     * @param {Integer} multiplier - required
     * @return {String} returns string in format: P0DT0H0M0S
    */
    podMultiplier : function(pod, factor){
        var days    = parseInt(pod.substring(pod.indexOf('P')+1,pod.indexOf('D')));//days
        var hours   = parseInt(pod.substring(pod.indexOf('T')+1,pod.indexOf('H')));//hours
        var minutes = parseInt(pod.substring(pod.indexOf('H')+1,pod.indexOf('M')));//minutes
        var seconds = parseInt(pod.substring(pod.indexOf('M')+1,pod.indexOf('S')));//seconds
    ...
    //Flip tile idx from decarta (0.0 center) to normal (0.0 top left)
    normalizeTile: function(x,y,z){
    ...
    //flip tile idx from normal to decarta
    denormalizeTile: function(x,y,z){
...
deCarta.Window = {    
    ...
    */
    ...
    /**
     * Returns ie version or undefined if not IE.
     */
    whichIe:function(){
    ...
    hasCSSTransforms: function(){
        //Chrome 24 has totally broken css tranforms. 
...
deCarta.Core.EventableObject.prototype = {
	...
	addListeners: function(element, opaque){
        ...
        deCarta.Touch.attachListener('touchmove', this.owner.owner.touchLayer, this.touchMove.bind(this), opaque);
        deCarta.Touch.attachListener('touchend', this.owner.owner.touchLayer, this.touchEnd.bind(this), opaque);        
    ...
    checkEvent: function(e, pos){
        ...
        var x = e.pageX - this.owner.owner.domX - this.domX;
        var y = e.pageY - this.owner.owner.domY - this.domY;  
 ...
 *       default=true</li>
 *   <li>(bool) autoResize: If <em>true</em>, the map will resize automatically when the container is resized. Optional,
 *       default=true</li>
 *   <li>(bool) scrollWheelEnabled: If <em>true</em>, the scroll wheel will serve as a zoom control. Optional,
 *       default=true</li> 
...
deCarta.Core.Map = function(options){
    ...
    this.canvas.className = 'deCarta-Map-V3';
    ...
    this.layerContainer = deCarta.crEl('div');
    this.layerContainer.className = 'deCarta-Map-Layers';
    ...
    this.ovlContainer = deCarta.crEl('div');
    this.ovlContainer.className = 'deCarta-Map-Overlays';
    this.ovlContainer.style.position = 'absolute';
    this.ovlContainer.style.top = 0;
    this.ovlContainer.style.left = 0;
    this.ovlContainer.style.width = '100%';
    this.ovlContainer.style.height = '100%';   
    this.ovlContainer.style.zIndex = 100; 

    //The reason for this is the combination of iOS Safari and IE    
    //In iOs, during pinch zooming, if the contents of the 
    //element receiving the events change the events stop firing.
    //Completely. Therefore we had to add a special iOS 
    //touch layer that never changes and receives all these events. 
    //in IE, thanks to errors in the propagation of events depending
    //on z-indexing, this causes it to break. So this is the solution
    //It's not ideal, but it works. 
    if (deCarta.Window.isIOS()) this.touchLayer = this.ovlContainer;
    else this.touchLayer = this.canvas;

    this.canvas.appendChild(this.layerContainer);    
    this.canvas.appendChild(this.ovlContainer);
    ...
    if (this.options.skipBindingToServer){
        ...
        deCarta.WebServices.init(function (resp) {            
            if (resp.success){                
                this.init();
            } else {
...
deCarta.Core.Map.prototype = {
    ...
    resize: function(){
        ...
        var pos = deCarta.Utilities.domPosition(this.containerElement);
    ...
    */
    ...
    init: function(){        
        ...
        this.streetLayer = new deCarta.Core.MapLayer({
            name: 'Map View',
            visible: true,
            tileStore: new deCarta.WebServices.StreetTileStore({
        ...
        this.hybridLayer = new deCarta.Core.MapLayer({
            name: 'Hybrid View',
            visible: false,
            tileStore: new deCarta.WebServices.TransparentTileStore({
                precache: false
            })
        });
        this.addLayer(this.hybridLayer); 

        this.trafficLayer = new deCarta.Core.MapLayer({
            name: 'Traffic View',
            visible: false,
            opacity: 60,
            tileStore: new deCarta.Core.TrafficTileStore({
                precache: false
            })
        });
        ...
        if (!this.options.avoidEvents) {
            this.addListeners(this.touchLayer, true);
        }
    ...
    addLayer: function(layer){
        ...
        var dest = (layer.type == 'layer')  ? this.layerContainer : this.ovlContainer;
    ...
    */
    showTraffic: function(){
        ...
        this.render();
        //this.trafficLayer.setOpacity(66.6);
        this.triggerEvent('viewchange',{view: 'traffic'});
    },
    /**
    * hide traffic
    */
    ...
    touchEnd: function(e, oe){
        ...
        this.startZoom = null;
        if (Math.ceil(this.zoom) != this.zoom ){
            //need to get it to the right int zoom like that
            var targetZoom = Math.round(this.zoom);
            this.zoomTo(targetZoom, this.lastPinchCenter);
        }
    ...
    touchMove: function(e, oe){
        ...
        if (e.scale && e.scale != 1){
            ...
            this.lastPinchCenter = fixedPoint;
            this.zoom = tZoom;
...
deCarta.Core.MapLayer.prototype = {
    ...
    */
    render: function(tileGrid){
        ...
        //console.log('Purging load requests');
...
deCarta.Core.MapOverlay = function(options){
    ...
    this.options = {
        ...
        clusteringOptions:{},
        useDOMElementRemoval: true,
        onClusterClick: function(cluster){}
    }
    this.type ='overlay';
    this.options = deCarta.Utilities.extendObject(this.options, options);

    if (deCarta.Window.whichIe() < 10)
        this.options.useDOMElementRemoval = false;
}
...
deCarta.Core.MapOverlay.prototype = {
    ...
    setOpacity: function(opacity){
        ...
        deCarta.Utilities.setOpacity(this.domElement, opacity);
    },    
    ...
    render: function(tileGrid){
        ...
        if (this.options.useDOMElementRemoval) var restore = deCarta.Utilities.removeElementToReinsert(this.domElement);                  
        ...
        if (this.options.useDOMElementRemoval) restore();
    ...
    renderShapes: function(screenX, screenY, tiles){
        ...
        for (var shapeId in this.visibleShapes){
            if (!keepList[shapeId]) {
                ...
                if (shape.unrender) shape.unrender();
    ...
    renderPins: function(screenX, screenY, tiles){        
        ...
        for (i = 0; i < renderList.length; i++){
            if (!renderList[i].parentNode || !renderList[i].parentNode.id){
    ...
    callChildren: function(event, e, oe, alternateEvent){
        ...
        var x = (e ? e.pageX : 0) - mapPosition.left;
        var y = (e ? e.pageY : 0) - mapPosition.top;            
        var eventPosition = this.owner.owner.positionFromXY(x, y);
...
deCarta.Core.Configuration = {
    ...
    /** Tile Config used in traffic tiles */
    defaultTrafficConfig: 'traffic-usa',
    ...
    DGUrlVersion: 2,
    digitalGlobeProfile: 'Consumer_Profile',
...
deCarta.Core.ZoomFramer.prototype = {
    ...
    init : function(){
        ...
        this.dom.className = 'deCarta-ZoomFramer'
...
deCarta.Core.InfoWindow = function(map){
    ...
    //we have to let touch end go through so that the map can stop scrolling
    //JP MAPIT bug 9/6/13
    deCarta.Touch.attachListener('touchend', this.domElement, this.touchEnd.bind(this), true);
    /** DOMMouseScroll is for mozilla. */
...
deCarta.Core.InfoWindow.prototype = {
    ...
    position: function(map){
        ...
        if (this.autoReorient) {
            //dock bottom only if not enough space on top (top and bottom mean the opposite of what you think) are you confused yet?
...
deCarta.Core.TileStore = function(opts){
    ...
    this.tileCacheSize = deCarta.Window.isMobile() ? 100 : 400;
    ...
    //setInterval(this.checkQueue.bind(this), 100);
}
...
deCarta.Core.TileStore.prototype  = {
    ...
    getTile: function(x, y, z){       
        ...
        //the tile is loading already, pass the next best for now
        if (this.loadingTiles[key]) {
            return this.getNextBestTile(x,y,z);
        }

        //we don't have it. let's load it. 
        this.queueTile(x, y, z);
        return this.getNextBestTile(x,y,z);
    },

    queueTile: function(x, y, z){
        var key = this.getKey(x,y,z);
        this.loadingTiles[key] = true;
        if (Object.keys(this.loadRequests).length < this.opts.concurrency){
            this.loadTile(x, y, z);
        }
        this.loadQueue[key] = {x: x, y: y, z: z};
    ...
    checkQueue: function(){
        ...
        try {
        var requestKeys = Object.keys(this.loadRequests);
        } catch (e){}
    
        if (!requestKeys) return;
        
        while (requestKeys.length < this.opts.concurrency){
                    
            var keys = Object.keys(this.loadQueue);
            if (keys.length == 0) return;
            var key = keys.shift();
            //console.log(key);

            var t = this.loadQueue[key];
            this.loadTile(t.x, t.y, t.z);
            this.loadQueue[key] = null;
            delete this.loadQueue[key];
    ...
    loadTile: function(x, y, z){
        ...
        tile.onload = function(t){
            ...
            setTimeout(this.checkQueue.bind(this),1);
        }.bind(this, tile);
    ...
    purgeLoadRequests: function(centerTile, zoom, radius){
        ...
        var keys = Object.keys(this.loadRequests).concat(Object.keys(this.loadQueue));
...
deCarta.Core.OverlayObject = function(opts){
    ...
    //let's see if we can drop svg support in 3.0.9
    //why? canvas works, and we have less maint. it was chrome only anyways
...
deCarta.Core.OverlayObject.prototype = {
    ...
    elementSpecs: function(x,y,z){

        var size = this.getSize(z);
        var x = (x - this.padding);
        var y = (y - this.padding);
        ...
        if (x + size.width > this.owner.owner.width){
            size.width = this.owner.owner.width  - x;
        }
        if (y + size.height > this.owner.owner.height){
            size.height = this.owner.owner.height  - y;
        }   

        size.width += this.padding * 2;
        size.height += this.padding * 2;
 ...
 * It can have a custom image, and when clicked it can display custom text.
 ...
 *  <li>(function) onShowText: this function will be invoked when the pin's infowindow is shown</li> 
 *  <li>(function) onHideText: this function will be invoked when the pin's infowindow is hidden</li> 
 *  <li>(function) onBeforeShowText: this function will be invoked before the pin's infowindow is shown - returning false will prevent the infowindow from being displayed.</li> 
 *  <li>(function) onBeforeHideText: this function will be invoked before the pin's infowindow is hidden - returning false will prevent the window from closing</li>  
 *  <li>(function) onImageLoad: invoked when the pin's image completes loading</li>  
 *  <li>(string) cssClass: this css class will be added to the pin. Can be used for styling</li>   
 *  <li>(deCarta.Core.InfoWindow) infoWindow: If you require the pin to have it's own infoWindow, instead of using the global object, you can pass an instance of {deCarta.Core.InfoWindow} in this property</li> 
...
deCarta.Core.Pin = function(opts){
    ...
    this.options = {
        ...
        onShowText: null,
        onHideText: null,
        onBeforeShowText: null,
        onBeforeHideText: null,
        onImageLoad: null,
        cssClass: '',
        label: '',
        opaque: false,
...
deCarta.Core.Pin.prototype = {
    ...
    _render: function(x, y, skipListeners){        
        ...
        if (this.options.imageSrc){
            ...
            this.pinImage.onload = function(){
                ...
                if (self.options.onImageLoad) self.options.onImageLoad(self);
            };
        ...
        //fixes missing onclick
        this.onclick(function(){ if (this.options.onClick) this.options.onClick() }.bind(this))
        //put image in main element,        
        this.domElement.appendChild(this.pinImage);

        deCarta.Core.EventManager.listen('moveend', function(){            
            if (this.textVisible 
                && this.getInfoWindow().domElement.style.display!='none') {
                this.getInfoWindow().position();
            }
    ...
    tap: function(e, oe){
        ...
        if (this.options.text){ // only open window if there is something to show                        
            if (this.textVisible){
                if (this.options.onBeforeHideText) if (!this.options.onBeforeHideText(this)) return;
                this.hideText();
            } else {
                if (this.options.onBeforeShowText) if (!this.options.onBeforeShowText(this)) return;
                this.showText();
            }
        }
    },
    ...
    /**
     * If the pin is clustered, this will return the cluster it belongs to. Otherwise returns null.
     */
    getCluster : function(){
    ...
    getInfoWindow: function(){
        return this.options.infoWindow || this.owner.owner.infoWindow;
    },
    ...
    showText: function(){
        ...
        var el = this.getInfoWindow().getElement();
 ...
 *  <li>strokeLineCap: <b>NOTE: SVG Rendering only</b> Style for the line endings. Can be one of : "butt", "round", "square" (see http://www.w3.org/TR/SVGTiny12/painting.html)</li>
 *  <li>strokeLineJoin: <b>NOTE: SVG Rendering only</b> Shape which shall be used at the corners of shapes when they are stroked. Can be one of: "miter", "round, "bevel". (see http://www.w3.org/TR/SVGTiny12/painting.html)</li> 
 * </ul>
 *
 * @see deCarta.Core.OverlayObject
 * @see deCarta.Core.MapOverlay
...
deCarta.Core.Polyline = function(options){
    ...
    if (typeof this.options.lineGeometry[0] == 'object' && this.options.lineGeometry[0].getLat){
        ...
        for(var i = 0; i< this.options.lineGeometry.length; i++){
            ...
            var tPos = new deCarta.Core.Position(parseFloat(ll[0]), parseFloat(ll[1]));
    ...
    */    
    ...
    toJSON: function(){
        ...
        for ( i = 0; i < this.positions.length; i++ ) {
            obj.lineGeometry.push(this.positions[i]);
        }
        return obj;
    },    
    ...
    renderVML: function(x, y, z, tiles){
        if (!this.container){
            ...
            this.container.unrender = function(){
                this.container = null;
            }.bind(this)             
        ...
        for (var i = 0; i < points.length; i++){
            ...
            if ( p.op == 'm'){
                if (pStr.length > 0){
                    vmlStr += '<v:polyline points="' + pStr + '">'+strokeStr+ fillStr +'</v:polyline>';
                    pStr = '';
                }
            } else {
    ...
    renderSVG: function(x, y, z, tiles){
        ...
        var specs = this.elementSpecs(x, y, z);
        ...
        //console.log('rend', points)
        for (var i = 0; i < points.length; i++){
            
            var p = points[i];
            if (p.op == 'm'){
                //make new line
                if (pStr.length > 0 && lineElement){
                    //append
                    lineElement.setAttribute('points', pStr);
                    lineElement.setAttribute('style', "stroke-linecap:"+this.options.strokeLineCap+";stroke-linejoin:"+this.options.strokeLineJoin+";fill:none;stroke:"+this.options.strokeColor+";stroke-width:"+this.options.strokeWidth+";stroke-opacity:"+this.options.strokeOpacity);
                }
                lineElement = document.createElementNS("http://www.w3.org/2000/svg", "polyline");
                pStr = '';
                this.svgElement.appendChild(lineElement);
            } else {
                pStr += (p.x - specs.xOfs) + ',' + (p.y - specs.yOfs) + ' ';
            }
        }
        ...
        this.svgElement.setAttribute('style', "position: absolute; width: " + (specs.width) + "px; height: " + (specs.height) + "px;");
        this.svgElement.style.zIndex = this.zIndex;
        this.domPosition(specs.x, specs.y,  this.svgElement);
    ...
    renderCanvas: function(x, y, z, tiles){
        ...
        if (specs.width != this.lastSpecs.width || 
            specs.height != this.lastSpecs.height || 
            specs.xOfs != this.lastSpecs.xOfs ||
            specs.yOfs != this.lastSpecs.yOfs) {

            this.lastSpecs = specs;

            
            this.canvasElement.width = specs.width;
            this.canvasElement.height = specs.height;

            var dX = this.topLeftPoint.getX(z) + specs.xOfs;
            var dY = this.topLeftPoint.getY(z) - specs.yOfs;
        ...
        this.domPosition(specs.x, specs.y, this.canvasElement);
    ...
    try {
        ...
        var ty = 0;
        var distance = null;
        if (!this.segmentIndex[nz]){   
            this.segmentIndex[nz] = {};
           /*
             the new point index!

             using edges this time. 
             each edge is indexed based on all tiles that it crosses
             when preparing points, we get the affected edges from the edge index,
             and create a list of instructions - move to, line to depending on if they are connected or not. 
             this is passed to the render function which then, yes, renders it. 
           ...
           */
            ...
            for (var i = 0; i < posLength; i++){
                ...
                if (distance > this.options.granularity){
                    var lastPoint = {x: tx, y: ty};
                    
                    var segment = new deCarta.Core.Segment(lastPosition, p);
                    //now index this segment as needed
                    var tiles = segment.getCrossedTiles(nz);
                    segment.idx = segCount;
                    segCount ++;
                    for (var j = 0; j < tiles.length; j++){
                        if (!this.segmentIndex[nz][tiles[j]]) this.segmentIndex[nz][tiles[j]] = []
                        this.segmentIndex[nz][tiles[j]].push(segment);
                    }
                    var lastPosition = p;
                }
    ...
    preparePoints: function(z, dX, dY, tiles){            
        ...
        //lets start by returning the right stuff like that
        //then we split them up and return structs
       ...
       // debugger;
        ...
        for (var i = 0; i< tiles.length; i++){
            var key = deCarta.Utilities.getTileKey(tiles[i].N, tiles[i].E, nz);
            
            if (this.segmentIndex[nz][key]){   
                //console.log(key, this.segmentIndex[nz][key])             
                for (var j = 0; j < this.segmentIndex[nz][key].length; j++){
                    var seg = this.segmentIndex[nz][key][j];                    
                    if (!uniques[seg.idx]){
                        segs.push(seg);
                        uniques[seg.idx] = true;
                    }    else {
                        throwed ++
                    }
                }
        ...
        //console.log(segs.length, throwed);
        ...
        for (var i = 0; i < segs.length; i ++){
            ...
            px = seg.a.getX(z) - dX + this.padding;                    
            py = dY - seg.a.getY(z) + this.padding;

            if (lastIdx + 1 < seg.idx){
                points.push({x: px, y: py, i: seg.idx, op: 'm'})
            }
            
            points.push({x: px, y: py, i: seg.idx, op: 'l'});
        

            px = seg.b.getX(z) - dX + this.padding;                    
            py = dY - seg.b.getY(z) + this.padding;
...
deCarta.Core.Segment = function(a, b){
    
    this.a = a;
    this.b = b;
}

deCarta.Core.Segment.prototype = {

    //get the tiles it crosses by zoom level
    getCrossedTiles: function(z){

        z = deCarta.Utilities.normalizeZoom(z);

        var aTile = this.a.getTileAtZoom(z);
        var bTile = this.b.getTileAtZoom(z);

        var aKey = deCarta.Utilities.getTileKey(aTile.N, aTile.E, z);
        var bKey = deCarta.Utilities.getTileKey(bTile.N, bTile.E, z);

        if (aKey == bKey) return [aKey];

        //if we are here, well. Figure out the affected tiles and return them.

        var x0 = Math.min(aTile.E, bTile.E);
        var x1 = Math.max(aTile.E, bTile.E)
        ...
        /*var y0 = Math.min(aTile.N, bTile.N);
        var y1 = Math.max(aTile.N, bTile.N);*/
        ...
        while(true){
            ...
            var eK = deCarta.Utilities.getTileKey(y0, x0, z);            
            if (!tl[eK]){
                tl[eK] = true;
                ret.push(eK);
            }
    ...
    renderSVG: function(x, y, z, tiles){
        ...
        for (var i = 0; i < points.length; i++){
            ...
            if (p.op == 'm'){
                ...
                if (!this.lineElements[lineElementIdx]){
                    //console.log('Creating element: ', lineElementIdx);
                    this.lineElements[lineElementIdx] = document.createElementNS("http://www.w3.org/2000/svg", "polyline")                                        
                    this.svgElement.appendChild(this.lineElements[lineElementIdx]);
                } 
                lineElement = this.lineElements[lineElementIdx];                
        ...
        if (pStr.length > 0 && lineElement){
            if (!this.lineElements[lineElementIdx]){
                //console.log('Creating element: ', lineElementIdx,pStr, lineElement);
                this.lineElements[lineElementIdx] = document.createElementNS("http://www.w3.org/2000/svg", "polyline")                                        
                this.svgElement.appendChild(this.lineElements[lineElementIdx]);
            } 
            lineElement = this.lineElements[lineElementIdx];
        ...
        //console.log('Render complete, elements used : ', lineElementIdx, 'out of ',  this.lineElements.length)
        if (lineElementIdx < this.lineElements.length){
            for (var i = lineElementIdx; i < this.lineElements.length; i ++){
                this.lineElements[i].setAttribute('points', '');
            }
        }           
...
deCarta.Core.Polygon = function(options){
    ...
    //make sure they are positions like so
    for (var i = 0; i < this.options.vertices.length; i ++){
        
        if ((typeof this.options.vertices[i]).toLowerCase() === 'string'){
            this.options.vertices[i] = new deCarta.Core.Position(this.options.vertices[i]);
        } 
    }
...
deCarta.Core.Polygon.prototype = {
    ...
    */    
    ...
    toJSON: function(){
        ...
        for ( i = 0; i < this.options.vertices.length; i++ ) {
            obj.vertices.push(this.options.vertices[i]);
        }
        return obj;
    },
    /**
     ...
     */
    getSize: function(z){        
        var bbSize = this.boundingBox.getSize(z);
    ...
    */
    ...
    renderCanvas: function(x, y, z, tiles){                
        ...
        var newW = specs.width ;//+ (this.padding * 2);
        var newH = specs.height;// + (this.padding * 2);
        
        if (newW != this.canvasElement.width || newH != this.canvasElement.height || specs.xOfs != this.lastxOfs || specs.yOfs != this.lastyOfs) {

            this.lastxOfs = specs.xOfs;
            this.lastyOfs = specs.yOfs;
...
/*        var zlit = 4;
        if (z > zlit){
            //transform x and y to z 15 x and y
            var p = new deCarta.Core.Position(0,0);
            ...
            var x15 = p.getX(zlit);
            var y15 = p.getY(zlit);            
            var specs = this.elementSpecs(x15,y15,zlit);                    
        }*/
        
        var absPos = this.getPosition().getPixelPoint(z);
                        
        var newW = specsx.width;// + (this.padding * 2);
        var newH = specsx.height;// + (this.padding * 2);                        
        ...
        var strokeStr = '<v:stroke opacity="' + Math.round(this.options.strokeOpacity * 100) + '%" weight="'+ this.options.strokeWidth+'px" color="'+this.options.strokeColor+'" />';
        var vmldata = '<v:shape coordsize="'+ specs.width +',' + specs.height + '" style="margin:0;padding:0;position:absolute;top:0;left:0;width:'+ newW + 'px;height:' + newH + 'px;" path="' + vStr + '"> ' + fillStr + strokeStr + '</v:shape>';
    
        this.domPosition(specsx.x, specsx.y, this.container);
    ...
    renderSVG: function(x, y, z, tiles){
        ...
        var specs = this.elementSpecs(x,y,z);
        var newW = specs.width ;
        var newH = specs.height;
        ...
        this.domPosition(specs.x, specs.y, this.container);
...
deCarta.Core.Circle = function(options){
    ...
    if ((typeof this.options.position).toLowerCase() === 'string'){
        this.options.position = new deCarta.Core.Position(this.options.position);
    } 
...
deCarta.Core.Circle.prototype = {
    ...
    */
    ...
    renderCanvas: function(x, y, z, tiles){
        ...
        var specs = this.elementSpecs(x -pxRadius, y - pxRadius,z);
    
        if (!this.lastSpecs) this.lastSpecs = {}
        this.domPosition((specs.x), (specs.y), this.canvasElement);
        ...
        var newWidth = specs.width;//(pxRadius * 2) + (this.padding * 2);
        var newHeight = specs.height; //(pxRadius * 2) + (this.padding * 2);

        if (this.canvasElement.width != newWidth || 
            this.canvasElement.height != newHeight ||
            this.lastSpecs.xOfs != specs.xOfs ||
            this.lastSpecs.yOfs != specs.yOfs){
            //if we are here, we are in view. So... what do we do
            this.canvasElement.width = specs.width;//(pxRadius * 2) + (this.padding * 2);
            this.canvasElement.height = specs.height;//(pxRadius * 2) + (this.padding * 2);

            var ctx = this.canvasElement.getContext("2d");

            //draw the circle
            ctx.beginPath();
            ctx.arc(pxRadius - specs.xOfs + this.padding, pxRadius - specs.yOfs + this.padding, pxRadius, 0, Math.PI*2, true);
            ctx.closePath();
    ...
    renderSVG: function(x, y, z, tiles){
        ...
        this.circleElement.setAttribute('cx', pxRadius - specs.xOfs + this.padding);
        this.circleElement.setAttribute('cy', pxRadius - specs.yOfs + this.padding);
    ...
    renderVML: function(x, y, z){
        ...
        if (!this.container) {        
            ...
            this.container = deCarta.crEl('div');
            this.container.id = ("deCarta-circle-container-" + Math.random()).replace(".", "");            
            this.shapeId = ("deCarta-circle-" + Math.random()).replace(".", "");            
        ...
        var fillStr = '<v:fill opacity="' + Math.round(this.options.fillOpacity * 100) + '%" color="'+this.options.fillColor+'"/>'
        var strokeStr = '<v:stroke opacity="' + Math.round(this.options.strokeOpacity * 100) + '%" weight="'+ this.options.strokeWidth+'px" color="'+this.options.strokeColor+'" />';        
        var vmlStr = '<v:oval id="'+this.shapeId +'" style="position: absolute;top:0;left:0;width:'+newWidth+'px;height:'+newHeight+'px"> ' + fillStr + strokeStr + '</v:oval>';
        
        this.container.innerHTML = vmlStr;
        
        this.domPosition((x - this.padding - pxRadius), (y - this.padding - pxRadius), this.container);
        var elid = this.shapeId ;
        /*setTimeout(function(){*/
    ...
    if (lat != null) {
        if (arguments.length == 1){
            if (typeof lat == 'string'){
                ...
                this.lat = parseFloat(lat.getLat());
                this.lon = parseFloat(lat.getLon());
            } else {
...
deCarta.Core.Position.prototype = {
     ...
     * @param {deCarta.Core.Position} another
    ...
    /*
    * Returns the distance between this position and
    * the one passed as the argument.
    * @param {deCarta.Core.Position} another
    * @return {deCarta.Core.Distance} 
    */
    distanceFrom: function(another){
        var d = deCarta.Utilities.getPointDistance(this, another);
        return new deCarta.Core.Distance(d, 'KM');
    },
     ...
     */
    toString: function(n, sep){
        if (!sep) sep = " ";
        if (n) return this.lat.toFixed(n) + sep + this.lon.toFixed(n);        
        return this.lat + sep + this.lon;
    },
...
/**
*	@class
*	The Time class defines a structure for easily manipualting time values,
*	for instance those returned by a routing request, or a corridor search.
*
* @constructor
* @param {float} time value
* @param {string} uom Unit of measure. valid values are : S (Seconds), M (Minutes), H (Hours), D (days)
*/
deCarta.Core.Time = function(value, uom){	

	if (typeof value == 'object'){
		if (value.value && value.uom){
			this.value = value.value;
			this.uom = value.uom;
			return this;
		}
		throw ('Unrecognized format for time value')
	}
...
deCarta.Core.StructuredAddress.prototype = {
    ...
    fromWSResponse: function(r){
        ...
        if (r.PostalCode && typeof r.PostalCode === 'string'){
            try {
                this.postalCode = r.PostalCode.replace(/\"/g, "");
            } catch (e){
                //error with the postal code, sometimes it has double quotes,
                //or maybe it is not a string, many things seem to happen to it 
                //from time to time. Better be safe. 
            }
        }        
...
deCarta.Core.RouteCriteria.prototype = {
     ...
     * "Pedestrian". 
     * It can either be a single value (eg PassengerCar) or a comma separated list
     * (eg PassengerCar,4WD) or an array (eg ["PassengerCar", "4WD"])
     * Defaults to PassengerCar
     */
     ...
     * Ferry /FerryBoat / FerryTrain
     * Toll / TollRoad / TollArea
     * Tunnels
     * Bridges
     * UnpavedRoad
     * Skyway
    ...
    * a URL to a small image representing the maneuver in a top up
    * orientation
    * @type String URL
    * @default false
    */
    ...
    */
    maneuverMapConfig : {
        ...
        lineWidth: 8,
        lineOpacity: 60,
        lineStyle: 'SOLID' //SOLID, DASH, DOT, DASHDOTDOT, ALTBLACKDASH, RAIL, NONE
    },    

    toQueryString: function(){
        return 'criteria='+JSON.stringify(this);
    }
...
deCarta.Core.HTMLRenderer = function(container){
    ...
    this.imageWrapper.style.zIndex = 1;
...
deCarta.Core.HTMLRenderer.prototype = {
    ...
    setOpacity: function(opacity){
        ...
        deCarta.Utilities.setOpacity(this.imageWrapper, opacity);
    },    
...
deCarta.WebServices = {
	...
	/**
	* Initialize the web service layer. 
	*/
	init: function(callback){
		deCarta.WebServices.JSRequest.init(callback);
	}
}
...
deCarta.WebServices.JSRequest = {
        ...
        this.send(xmlRequest, function(res){
            ...
            try {
                if (typeof res.XLS.ResponseHeader.ErrorList === 'object'){
                    ...
                    //console.log('INIT INIT ', res.XLS.Response.RUOKResponse.hostName)
    ...
    send: function(xml, onSuccess, onFailure, getRaw){
        ...
        '&callback=window.deCarta.WebServices.JSRequest.callbacks[' + reqId + ']'+
        '&data=' + data +
        '&responseFormat=JSON';

        //try both a script, and a node http client 
        if (document.createElement){
            ...
            callback = function(response) {
              ...
              response.on('end', function () {
                ...
                for (var type in response.response.XLS.Response){
                    //console.log('TYPE: ', type, response.response.XLS.Response)
                    if (this.interceptors[type])
                    for (var i = 0; i < this.interceptors[type].length; i++){
                        if (typeof this.interceptors[type][i] === 'function') 
                            response.response = this.interceptors[type][i](response.response);
                ...
                onSuccess(response.response);
                return;            
              }.bind(deCarta.WebServices.JSRequest));
            }
...
//        }
        ...
        return deCarta.WebServices.JSRequest.send(this.getXML(criteria), this.handleResponse.bind(this,callback), this.handleFailure.bind(this, callback), getRaw);
...
deCarta.WebServices.JSRequest.addInterceptor('DirectoryResponse', deCarta.WebServices.POISearch.responseInterceptor);
/**
     ...
     * <pre>
{
  ...
  "accuracy": float
    ...
    geocode: function(address, callback){
        ...
        deCarta.WebServices.JSRequest.send(this.getGeocodeXML(address), this.handleResponse.bind(this, callback), this.handleFailure.bind(this, callback));
	 ...
	 * is as follows:
{
  ...
  "Position": deCarta.Core.Position
    ...
    reverseGeocode: function(position, callback, preference){
        ...
        deCarta.WebServices.JSRequest.send(this.getRevGeocodeXML(position, preference), this.handleRevResponse.bind(this, callback), this.handleFailure.bind(this, callback));
    },
     ...
     * @private
     */
    handleFailure: function(callback, error){
        callback(error)
    }
...
<xls:XLS version='1' xls:lang='en' xmlns:xls='http://www.opengis.net/xls' rel='4.5.1.sp02' xmlns:gml='http://www.opengis.net/gml'>
      ...
      <xls:Request maximumResponses='10' version='1.0' requestID='3332721' methodName='DetermineRouteRequest'>
        ...
        var start = new deCarta.Core.Position(criteria.waypoints.shift());
        var end = new deCarta.Core.Position(criteria.waypoints.pop());        
 ...
 * </pre>	 
    ...
    execute: function(criteria, callback){
        ...
        deCarta.WebServices.JSRequest.send(this.getXML(criteria), this.handleResponse.bind(this,callback), this.handleFailure.bind(this, callback));
...
deCarta.WebServices.JSRequest.addInterceptor('DetermineRouteResponse', deCarta.WebServices.Routing.responseInterceptor);
/**
 * @class
...
deCarta.WebServices.StreetTileStore = function(opt){
...
deCarta.WebServices.StreetTileStore.prototype = {
    ...
    getTileUrl: function(x,y,z){
        ...
        var port = deCarta.Core.Configuration.imagePort ? (deCarta.Core.Configuration.imagePort == 80 ? "" : ":"+deCarta.Core.Configuration.imagePort) : "";
        ...
        if (deCarta.Core.Configuration.urlVersion == 2) {
            var url = deCarta.Core.Configuration.streetTileHosts[this.hostIdx] +port+ '/openls/'+cache+'/TILE' +
                    ...
                    '&SESSIONID=' + deCarta.WebServices.JSRequest.sessionId +
                    '&CONFIG=' + config +
...
deCarta.WebServices.StreetTileStore.prototype = deCarta.Utilities.inherit(deCarta.WebServices.StreetTileStore.prototype, deCarta.Core.TileStore.prototype);
/**
 * @class
 * StreetTileStore extends the base TileStore to provide a standard street
 * map.
...
deCarta.WebServices.TransparentTileStore = function(opt){
    deCarta.Core.TileStore.call(this, opt);
    //a look up table providing the correct LLMAX values per zoom level.
    this._ll_LUT = deCarta.Core.Constants._ll_LUT;
    this.hostIdx = 0;
...
deCarta.WebServices.TransparentTileStore.prototype = {
    ...
    getTileUrl: function(x,y,z){
        ...
        if (deCarta.Core.Configuration.urlVersion == 2) {
            var url = deCarta.Core.Configuration.streetTileHosts[this.hostIdx] +port+ '/openls/'+cache+'/TILE' +
                    '/' + x + '/' + y + '/' + z + '?' + 
                    'CLIENTNAME=' + deCarta.Core.Configuration.clientName +
                    '&SESSIONID=' + deCarta.WebServices.JSRequest.sessionId + 'T' +
                    '&CONFIG=' + config +
                    '&P=' + deCarta.Core.Configuration.projection;
...
deCarta.WebServices.TransparentTileStore.prototype = deCarta.Utilities.inherit(deCarta.WebServices.TransparentTileStore.prototype, deCarta.Core.TileStore.prototype);
/**
 * @class
...
deCarta.WebServices.SatelliteTileStore = function(opt){
...
deCarta.WebServices.SatelliteTileStore.prototype = {
	

    getKey : function(x,y,z){
        //console.log(deCarta.Utilities.getTileKey(x,y,z) + deCarta.Core.Configuration.defaultConfig, this);
        return deCarta.Utilities.getTileKey(x,y,z) + '-DG-' + deCarta.Core.Configuration.DGUrlVersion + '-P-' + deCarta.Core.Configuration.digitalGlobeProfile;
    },  
    /**
	 * @private
        ...
        if (deCarta.Core.Configuration.DGUrlVersion == 2 && (deCarta.Core.Configuration.projection == 'EPSG:3857')) {
            if (deCarta.Core.Configuration.mapitImageryKey && z >= deCarta.Core.Configuration.mapitImageryFromZoomLevel && z <= deCarta.Core.Configuration.mapitImageryToZoomLevel) {
                ...
                var url = 'https://services.digitalglobe.com/earthservice/wmtsaccess?' +
                          ...
                          '&featureProfile=' + deCarta.Core.Configuration.digitalGlobeProfile +
...
deCarta.WebServices.SatelliteTileStore.prototype = deCarta.Utilities.inherit(deCarta.WebServices.SatelliteTileStore.prototype, deCarta.Core.TileStore.prototype);
/**
 * @class
 * TrafficTileStore extends the base TileStore to provide a traffic speed layer
 * @constructor
 * @param {object} opts Options (not currently used, for future expansion only)
 */

deCarta.WebServices.TrafficTileStore = function(opt){
    deCarta.Core.TileStore.call(this, opt);
    //a look up table providing the correct LLMAX values per zoom level.
    this._ll_LUT = deCarta.Core.Constants._ll_LUT;
    this.hostIdx = 0;
...
//Define methods to extend TrafficTileStore
deCarta.WebServices.TrafficTileStore.prototype = {
	

    getKey : function(x,y,z){
        //console.log(deCarta.Utilities.getTileKey(x,y,z) + deCarta.Core.Configuration.defaultConfig, this);
        return deCarta.Utilities.getTileKey(x,y,z) + '-' + deCarta.Core.Configuration.defaultTrafficConfig;
    },
    ...
    getTileUrl: function(x,y,z){
        ...
        var config = deCarta.Core.Configuration.defaultTrafficConfig;
        var port = deCarta.Core.Configuration.imagePort ? (deCarta.Core.Configuration.imagePort == 80 ? "" : ":"+deCarta.Core.Configuration.imagePort) : "";
          var dataSet = deCarta.Core.Configuration.dataSet ? deCarta.Core.Configuration.dataSet : "navteq-world";
        
        if (deCarta.Core.Configuration.urlVersion == 2) {
            var url = deCarta.Core.Configuration.streetTileHosts[this.hostIdx] +port+ '/openls/image/TILE' +
                    '/' + x + '/' + y + '/' + z + '?' + 
                    'CLIENTNAME=' + deCarta.Core.Configuration.clientName +
                    '&SESSIONID=' + deCarta.WebServices.JSRequest.sessionId + 'T' +
                    '&TDATA=true' +
...
}; //end TrafficTileStore prototype

//Extend the TileStore with the additional methods for TrafficTileStore
deCarta.WebServices.TrafficTileStore.prototype = deCarta.Utilities.inherit(deCarta.WebServices.TrafficTileStore.prototype,   deCarta.Core.TileStore.prototype);
//Wrapper for the old style 
if (deCarta.Core){
	deCarta.Core.Geocoder = deCarta.WebServices.Geocoder;
	deCarta.Core.Routing = deCarta.WebServices.Routing;
	deCarta.Core.POISearch = deCarta.WebServices.POISearch;
	deCarta.Core.StreetTileStore = deCarta.WebServices.StreetTileStore;
	deCarta.Core.TransparentTileStore = deCarta.WebServices.TransparentTileStore;
    deCarta.Core.TrafficTileStore = deCarta.WebServices.TrafficTileStore;
    deCarta.Core.SatelliteTileStore = deCarta.WebServices.SatelliteTileStore;
}
...
(function () {
    ...
    if (typeof module !== 'undefined' && module.exports) {
            module.exports = deCarta;
            root.deCarta = deCarta;
    } else {
            root.deCarta = deCarta;
    }

    root.encodeURIComponentUTF8 = function(F){
        F=F+"";
